<?php
require_once __DIR__ . '/../config/db.php';
$tests = $conn->query("SELECT test_name, COALESCE(description, '') AS description, price, COALESCE(test_category, 'General') AS test_category, COALESCE(sample_requirement, 'None') AS sample_requirement FROM tests WHERE status = 'Active' ORDER BY FIELD(test_category, 'Laboratory', 'Cardiology', 'Imaging', 'General'), test_name ASC");

$testRows = [];
if ($tests) {
    while ($row = $tests->fetch_assoc()) {
        $testRows[] = $row;
    }
}

$grouped = [];
foreach ($testRows as $row) {
    $category = trim((string)$row['test_category']) !== '' ? $row['test_category'] : 'General';
    if (!isset($grouped[$category])) {
        $grouped[$category] = [];
    }
    $grouped[$category][] = $row;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Test Prices - DiagnoLab</title>
    <link rel="stylesheet" href="/DiagnoSys/assets/css/style.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
</head>
<body>

    <!-- Navbar -->
    <nav class="navbar">
        <a href="../index.php" class="logo">
            <svg class="hero-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.75h4.5m-4.5 0v5.25a4.5 4.5 0 0 1-1.318 3.182L5.25 15.364a2.25 2.25 0 0 0 1.591 3.886h10.318a2.25 2.25 0 0 0 1.591-3.886l-3.182-3.182A4.5 4.5 0 0 1 14.25 9V3.75m-4.5 0h4.5" />
            </svg>
            DiagnoLab
        </a>
        <div class="nav-buttons">
            <a href="../auth/login.php" class="btn-outline">Login</a>
            <a href="../auth/register.php" class="btn-primary">Register</a>
        </div>
    </nav>

    <!-- Test Prices -->
    <section class="test-prices" id="test-prices">
        <div class="test-prices-nav">
            <a href="../index.php" class="btn-back-dashboard">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                Homepage
            </a>
        </div>
        <h2>Our Test Prices</h2>
        <p>Transparent pricing for all diagnostic tests</p>
        <?php if (!empty($grouped)): ?>
            <?php foreach ($grouped as $category => $rows): ?>
                <h3 style="max-width:1200px; margin:0 auto 14px; text-align:left; color:#0f172a;"><?php echo htmlspecialchars($category); ?></h3>
                <div class="price-grid" style="margin-bottom:26px;">
                    <?php foreach ($rows as $test): ?>
                    <div class="price-card">
                        <div class="price-card-icon"><i class="fa-solid fa-flask" style="font-size: 28px;"></i></div>
                        <h3><?php echo htmlspecialchars($test['test_name']); ?></h3>
                        <p class="price-desc"><?php echo htmlspecialchars($test['description']); ?></p>
                        <p class="price-desc" style="margin-bottom:10px;">
                            Type: <?php echo htmlspecialchars($test['test_category']); ?> | Sample: <?php echo htmlspecialchars($test['sample_requirement'] === 'None' ? 'No Sample Required' : $test['sample_requirement'] . ' Sample'); ?>
                        </p>
                        <div class="price-tag">৳<?php echo number_format($test['price'], 0); ?></div>
                        <a href="../auth/register.php" class="btn-primary">Book Now</a>
                    </div>
                    <?php endforeach; ?>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <div class="price-grid">
                <p class="no-tests">No tests available at the moment.</p>
            </div>
        <?php endif; ?>
    </section>

    <!-- Footer -->
    <footer class="footer">
        <p>&copy; 2026 DiagnoLab. All rights reserved.</p>
    </footer>

</body>
</html>
<?php $conn->close(); ?>


