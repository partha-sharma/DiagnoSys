...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($package_id > 0) {
        ...
        $packageTestsStmt = $conn->prepare('SELECT pt.test_id, pt.package_test_price, t.test_name, t.test_category, t.price AS master_price FROM package_tests pt INNER JOIN tests t ON t.test_id = pt.test_id WHERE pt.package_id = ? ORDER BY t.test_name ASC');
        ...
        while ($row = $packageTestsResult->fetch_assoc()) {
            ...
            $packageTestRows[] = [
                ...
                'test_category' => normalize_test_category((string)($row['test_category'] ?? 'General')),
    ...
    $bookedTestCategories = [];
    foreach ($packageTestRows as $row) {
        $bookedTestCategories[] = $row['test_category'] ?? 'General';
    }

    if (!empty($test_ids)) {
        $test_ids_str = implode(',', array_map('intval', $test_ids));
        $categoryRows = $conn->query("SELECT test_category FROM tests WHERE test_id IN ($test_ids_str)");
        if ($categoryRows) {
            while ($categoryRow = $categoryRows->fetch_assoc()) {
                $bookedTestCategories[] = normalize_test_category((string)($categoryRow['test_category'] ?? 'General'));
            }
        }
    }

    $bookedTestCategories = array_values(array_unique(array_filter($bookedTestCategories, static function ($category) {
        return $category !== '' && $category !== 'General';
    })));

    $preferredSpecialization = $bookedTestCategories[0] ?? 'Laboratory';
    $technicianValue = null;

    if ($assigned_technician_id > 0) {
        $chosenTechStmt = $conn->prepare('SELECT specialization FROM technicians WHERE technician_id = ? AND status = "Active" LIMIT 1');
        $chosenTechStmt->bind_param('i', $assigned_technician_id);
        $chosenTechStmt->execute();
        $chosenTech = $chosenTechStmt->get_result()->fetch_assoc();
        $chosenTechStmt->close();

        $chosenSpecialization = normalize_technician_specialization((string)($chosenTech['specialization'] ?? 'Laboratory'));
        if (in_array($chosenSpecialization, $bookedTestCategories, true)) {
            $technicianValue = $assigned_technician_id;
        }
    }

    if ($technicianValue === null) {
        $autoAssignStmt = $conn->prepare('SELECT technician_id FROM technicians WHERE status = "Active" AND COALESCE(specialization, "Laboratory") = ? ORDER BY technician_id ASC LIMIT 1');
        $autoAssignStmt->bind_param('s', $preferredSpecialization);
        $autoAssignStmt->execute();
        $autoAssign = $autoAssignStmt->get_result()->fetch_assoc();
        $autoAssignStmt->close();

        if ($autoAssign) {
            $technicianValue = (int)$autoAssign['technician_id'];
        }
    }
    ...
    try {
        ...
        if ($has_coupon_columns && $has_package_columns) {
            ...
            $stmt->bind_param("isissdsddissdi", $user_id, $appointment_datetime, $packageIdValue, $packageNameSnapshot, $packageSnapshot, $packagePriceSnapshot, $coupon_code, $discount_amount, $total_amount, $is_home_collection, $collectionAddressValue, $collectionDatetime, $collectionChargeValue, $technicianValue);
        } else {
